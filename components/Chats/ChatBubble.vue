<template>
  <div
    class="w-fit max-w-[70%] mt-2 px-2 pt-2 pb-1 rounded-lg text-black"
    :class="[
      chat?.sent_by === user?.id ? 'ml-auto' : '',
      chat?.message_type === 'TEXT' ? (chat?.sent_by === user?.id ? 'bg-red-300/60' : 'bg-slate-200') : 'bg-none',
    ]"
  >
    <div v-if="activeChatDetails?.chat_room?.participant?.length > 2" class="mb-1 flex gap-2 items-center">
      <fa-icon :icon="getRoleIcon(chat.sent_by)" class="text-sm text-slate-500" />
    ...
    <template v-if="chat?.message_type === 'IMAGE'">
      <img
        :src="chat?.message_attachment"
        alt="image"
        class="w-[260px] h-[260px] object-cover rounded-lg hover:cursor-pointer"
        @click="
          () => {
  ...
  >
    <template #modal-content>
      <div class="w-[800px]">
        <div
          class="w-full h-[600px] bg-contain flex bg-center bg-no-repeat justify-center items-center"
          :style="{
