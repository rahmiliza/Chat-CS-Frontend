<template>
  <div class="relative h-full w-[calc(100%-336px)]">
    ...
    <div class="h-14 w-full flex justify-between items-center border-b border-black px-2 pb-1">
      ...
      <decorators-permission-guard v-if="!activeChatData?.closed_at" permission="chat_room::add_participant">
      ...
      <decorators-permission-guard v-if="!activeChatData?.closed_at" permission="chat_room::finish">
    ...
    <div class="h-[calc(100%-56px)] w-full flex flex-col">
      <div ref="chatList" v-scroll="onScroll" class="h-full w-full overflow-y-auto flex flex-col-reverse px-4 mb-2">
        <template v-for="chat in activeChatDetails?.chats" :key="chat?.id">
          <ChatsChatBubble :chat="chat" :active-chat-details="activeChatDetails" />
        </template>
      </div>
      <decorators-permission-guard v-if="!activeChatData?.closed_at" permission="chat::store">
        <div class="flex gap-1 items-center pl-2">
          ...
          <input
            ...
            rows="1"
            @keydown.enter="() => handleRequestSendMessage('TEXT')"
          />
        </div>
      </decorators-permission-guard>
    </div>
  ...
  >
    <div class="relative flex flex-col w-fit h-fit bg-white rounded-lg justify-between shadow-xl">
      ...
      <div class="mt-4 pr-4 flex items-center justify-end">
        <fa-icon
          icon="xmark"
          class="text-2xl hover:cursor-pointer hover:text-red-500 active:text-red-700"
          @click="
            () => {
              uploadedFiles = []
            }
  ...
  <modals
    v-model="openModalAddNewParticipant"
    modal-title="Add Participant"
    :show-overflow="true"
    :on-ok="handleOkAddNewParticipant"
    :on-close-modal="handleCloseModalAddNewParticipant"
    ...
    <template #modal-content>
      <div class="w-[400px] h-[80px] select-none overflow-y-visible">
        ...
        <template v-else>
          ...
          <input-select
            v-model="selectedParticipantToAddToChat"
            placeholder="Choose one Admin"
...
import type { ChatRoom, ChatRoomDetails } from '~/models/chat'
...
async function handleRequestSendMessage(message_type: 'TEXT' | 'IMAGE' = 'TEXT') {
  ...
  try {
    ...
    const { data, error } = await useApi<UpsertResponse<Chat>>('/chats', {
      method: 'POST',
      body: {
        message: chatMessage ?? '',
        room_id: props?.activeChatData?.id ?? '',
